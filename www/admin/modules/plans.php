...
if($_GET['action'] == "add") {
	...
	if($name && $minsum && $percent && $days) {
		...
		if($file && $type == "png") {
			mysql_query("UPDATE plans SET img = ".$lid." WHERE id = ".$lid." LIMIT 1");
...
print "</td>";
	...
	if($row['status'] == 1) {
		print "	<td width=\"20\"><a href=\"?p=plans&id=".$row['id']."&status=0&act=cp\"><img src=\"images/no_view.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"На паузе / включить\" /></a></td>";
	} else {
		print "<td width=\"20\"><a href=\"?p=plans&id=".$row['id']."&status=1&act=cp\"><img src=\"images/view.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"Включен / выключить\" /></a></td>";
	}
	
	print "
	<td width=\"20\"><a href=\"?p=plan_edit&id=".$row['id']."\"><img src=\"images/edit.png\" width=\"20\" height=\"20\" border=\"0\" alt=\"Редактировать\" /></a></td><td width=\"20\"><a style=\"cursor:hand\" onclick=\"if(confirm('Вы действительно хотите удалить данный тарифный план?')) top.location.href='del/plans.php?id=".$row['id']."';\"><img src=\"images/delite.png\" width=\"20\" height=\"20\" border=\"0\" alt=\"Удалить\" /></a></td></tr>
<tr>
...
<form action="?p=plans&action=add" method="post" enctype="multipart/form-data">
<table width="700" bgcolor="#eeeeee" align="center" border="0" style="border: solid #cccccc 1px;">
	<tr>
	...
	<tr>
		...
		<td align="right"><input style="width: 302px;" type="text" name="percent" size="80" maxlength="10" value="" /><select name="period" id="period" onChange="checkPeriod();">
	...
	<tr>
		...
		<td align="right"><input style="width: 400px; border: 1px solid #999999; border-radius: 3px;" type="file" name="img"></td>
		<td width="18"><img style="cursor: help;" src="images/help_ico.png" width="16" height="16" border="0" alt="Данное изображение будет выводиться как иконка возле депозита. Можно использовать, например, для игры при покупке здания с его изображением. В некоторых случаях необходимо дать права на запись папке /img/depoico/" /></td>
	</tr>
	<tr>
